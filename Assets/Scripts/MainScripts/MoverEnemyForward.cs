...
public class MoverEnemyForward : MonoBehaviour
{
	...
	void Update()
	{
		if ((player.transform.position.z > levelStart) && (player.transform.position.z < levelStop))
		{
			...
			/*
			if (player.transform.position.z > min)
            {
				min = player.transform.position.z;
			}
			*/
			m_Collider.size = new Vector3(col, col, col);
			transform.position = new Vector3(transform.position.x, transform.position.y, direction*(Mathf.PingPong(Time.time * 20, max - min) + min));
		}
	}
